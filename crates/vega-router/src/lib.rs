...
impl ProjectConventions {
    ...
    pub fn state_type(&self) -> &str {
        if self.has_app { "crate::app::AppState" } else { "()" }
    }
}
...
pub fn detect_conventions(root: &Path, pages: &[RouteEntry]) -> anyhow::Result<ProjectConventions> {
    ...
    let middleware_fns = if has_middleware {
        ...
        let mut fns = Vec::new();
        for name in &["require_auth_page", "require_admin_page", "require_auth_api", "require_admin_api"] {
...
pub fn generate_all(
    ...
    generate_page_runtime_routes_file(root.join("pages"), &pages, out.join("runtime_pages.rs"), &conventions)?;
    generate_api_runtime_routes_file(root.join("api"), &api, out.join("runtime_api.rs"), &conventions)?;
...
pub fn generate_page_runtime_routes_file(
    ...
    let has_auth = conventions.middleware_fns.contains(&"require_auth_page".to_string());
    let has_admin = conventions.middleware_fns.contains(&"require_admin_page".to_string());

    let mut output = String::new();
    output.push_str("// AUTO-GENERATED BY vega-router. DO NOT EDIT.\n");
    output.push_str("#[allow(unused_mut, unused_variables)]\n");
    output.push_str(&format!("pub fn vega_register_page_runtime_routes(\n"));
    output.push_str(&format!("    mut app: vega::server::web::Router<{}>,\n", state_type));
    output.push_str(&format!("    state: {},\n", state_type));
    output.push_str(&format!(") -> vega::server::web::Router<{}> {{\n", state_type));
...
pub fn generate_api_runtime_routes_file(
    ...
    let state_type = conventions.state_type();
    let has_auth_api = conventions.middleware_fns.contains(&"require_auth_api".to_string());
    let has_admin_api = conventions.middleware_fns.contains(&"require_admin_api".to_string());

    let mut output = String::new();
    output.push_str("// AUTO-GENERATED BY vega-router. DO NOT EDIT.\n");
    output.push_str("#[allow(unused_mut, unused_variables)]\n");
    output.push_str(&format!("pub fn vega_register_api_runtime_routes(\n"));
    output.push_str(&format!("    mut app: vega::server::web::Router<{}>,\n", state_type));
    output.push_str(&format!("    state: {},\n", state_type));
    output.push_str(&format!(") -> vega::server::web::Router<{}> {{\n", state_type));
    ...
    for entry in entries {
        ...
        } else if has_auth_api && (entry.route_path == "/api/auth/me" || entry.route_path == "/api/auth/logout") {
...
pub fn generate_main_file(
    ...
    // Build router
    let state_type = conventions.state_type();
    output.push_str(&format!("    let mut router: vega::server::web::Router<{}> = vega::server::web::Router::new();\n", state_type));
    ...
    if !conventions.page_post_handlers.is_empty() {
        ...
        let has_admin_mw = conventions.middleware_fns.contains(&"require_admin_page".to_string());
        if has_admin_mw {
    ...
    output.push_str("    vega::server::run_with_shutdown(app, &config.server.host, config.server.port).await\n");
    output.push_str("}\n");
